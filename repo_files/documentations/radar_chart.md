...
|:---------------|:---------------|:-------|
|dataSets| list of [RadarDataSet](#RadarDataSet) that are shown on the radar chart|[]|
...
|radarBorderData| shows a border for the radar chart|BorderSide(color: Colors.black, width: 2)|
|getTitle| This function helps the radar chart to draw titles outside the chart. The default angle provided when called makes the title tangent to the radar chart. |null|
...
|tickCount|Defines the number of ticks that should be painted in RadarChart|1|
...
|PropName		|Description	|default value|
|:---------------|:---------------|:-------|
|dataEntries|Each RadarDataSet contains a list of [RadarEntries](#RadarEntry) that are shown in RadarChart.|[]|
...
|borderColor|Paints the DataSet border with the specified color|Colors.blueAccent|
|borderWidth|defines the width of [RadarDataSet](#RadarDataSet) border|2.0|
|entryRadius|defines the radius of each [RadarEntry](#RadarEntry)|5.0|

### RadarEntry
|PropName		|Description	|default value|
|:---------------|:---------------|:-------|
|value| RadarChart uses this field to render every point in the chart| null |
...
|touchSpotThreshold|the threshold of the touch accuracy. We find the nearest spots on the touched position based on this field.|10|
...
|touchedSpot|the [RadarTouchedSpot](#RadarTouchedSpot) that the user touched| null |

### RadarTouchedSpot
|PropName|Description|default value|
|:-------|:----------|:------------|
|touchedDataSet|the [RadarDataSet](#RadarDataSet) that the user touched| null |
|touchedDataSetIndex| index of the [RadarDataSet](#RadarDataSet) that the user touched| null |
|touchedRadarEntry|the [RadarEntry](#RadarEntry) that the user touched| null |
|touchedRadarEntryIndex| index of the [RadarEntry](#RadarEntry) that the user touched| null |

### RadarChartTitle
|PropName|Description|default value|
|:-------|:----------|:------------|
|text|the text of the title|required|
|children| A list of [InlineSpan](https://api.flutter.dev/flutter/painting/InlineSpan-class.html) that you can provide to have different texts with different styles. Just like how [TextSpan](https://api.flutter.dev/flutter/painting/TextSpan-class.html) works|null|
|angle|the angle used to rotate the title (in degrees)|0|
