## newVersion
* **BREAKING** (by @mahmoodhamdi) Remove non-functional `borderData` parameter from `PieChartData`. It was inherited from `BaseChartData` but never used by the pie chart painter. Use `PieChartSectionData.borderSide` instead for section borders, #1182
