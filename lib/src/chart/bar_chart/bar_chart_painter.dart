...
class BarChartPainter extends AxisChartPainter<BarChartData> {
  ...
  @visibleForTesting
  List<double> calculateGroupsX(Size viewSize, List<BarChartGroupData> barGroups,
      ...
      for (var j = 0; j < barGroup.barRods.length; j++) {
        ...
        if (barRod.toY != barRod.fromY) {
          if (barRod.toY > barRod.fromY) {
            ...
            final top = min(getPixelY(barRod.toY, drawSize, holder), bottom - cornerHeight);
            ...
            final bottom = max(getPixelY(barRod.toY, drawSize, holder), top + cornerHeight);
          ...
          if (barRod.rodStackItems.isNotEmpty) {
            for (var i = 0; i < barRod.rodStackItems.length; i++) {
              ...
              drawStackItemBorderStroke(canvasWrapper, stackItem, i, barRod.rodStackItems.length,
  ...
  void drawTitles(
    ...
    final leftInterval =
        leftTitles.interval ?? Utils().getEfficientInterval(viewSize.height, data.verticalDiff);
      ...
      AxisChartHelper().iterateThroughAxis(
        ...
        action: (axisValue) {
          if (leftTitles.checkToShowTitle(
            ...
            final span = TextSpan(
              style: Utils()
                  .getThemeAwareTextStyle(context, leftTitles.getTextStyles(context, axisValue)),
              text: text,
            );
    ...
    final rightInterval =
        rightTitles.interval ?? Utils().getEfficientInterval(viewSize.height, data.verticalDiff);
      ...
      AxisChartHelper().iterateThroughAxis(
        ...
        action: (axisValue) {
          if (rightTitles.checkToShowTitle(
            ...
            final span = TextSpan(
              style: Utils()
                  .getThemeAwareTextStyle(context, rightTitles.getTextStyles(context, axisValue)),
              text: text,
            );
            final tp = TextPainter(
              text: span,
    ...
    if (bottomTitles.showTitles && groupBarsPosition.containsAnythingToShow()) {
      ...
      for (var index = 0; index < groupBarsPosition.length; index += interval) {
        ...
        final span = TextSpan(
            style: Utils()
                .getThemeAwareTextStyle(context, bottomTitles.getTextStyles(context, xValue)),
            text: text);
        final tp = TextPainter(
            text: span,
  ...
  void drawTouchTooltip(
    ...
    final drawTooltipOnTop = tooltipData.direction == TooltipDirection.top ||
        (tooltipData.direction == TooltipDirection.auto && showOnRodData.toY >= 0);
  ...
  BarTouchedSpot? handleTouch(
    ...
    for (var i = 0; i < _groupBarsPosition!.length; i++) {
      ...
      for (var j = 0; j < groupBarPos.barsX.length; j++) {
        ...
        if (isPositive) {
          barTopY = getPixelY(targetData.barGroups[i].barRods[j].toY, chartViewSize, holder);
          barBotY = getPixelY(0, chartViewSize, holder);
        } else {
          barTopY = getPixelY(0, chartViewSize, holder);
          barBotY = getPixelY(targetData.barGroups[i].barRods[j].toY, chartViewSize, holder);
        }

        final backDrawBarY = getPixelY(
            targetData.barGroups[i].barRods[j].backDrawRodData.toY, chartViewSize, holder);
