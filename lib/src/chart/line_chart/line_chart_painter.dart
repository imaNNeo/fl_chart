...
class LineChartPainter extends AxisChartPainter<LineChartData> {
  ...
  void drawTouchedSpotsIndicator(
    ...
    for (final info in lineIndexDrawingInfo) {
      ...
      final pathData = indicatorLine.pathData ??
          FlPathData(
    ...
    if (barData.belowBarData.spotsLine.show) {
      for (final spot in barData.spots) {
        if (barData.belowBarData.spotsLine.checkToShowSpotLine(spot)) {
          ...
          final pathData = lineStyle.pathData ??
              FlPathData(
                // Kept for backwards-compatibility.
                // ignore: deprecated_member_use_from_same_package
  ...
  void drawBarShadow(
    ...
    final pathData = barData.pathData ?? const FlPathData();
    _barPaint
      ..strokeCap = pathData.strokeCap
      ..strokeJoin = pathData.strokeJoin
      ..strokeMiterLimit = pathData.strokeMiterLimit
    ...
    barPath = barPath.toDashedPath(pathData.dashArray);
